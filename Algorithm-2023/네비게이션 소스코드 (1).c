...
void recent_path_watch() {	// 최근 경로 출력 함수 
	FILE* fp = fopen("recent_path.txt", "r");
	char recent1[100][100];
	char recent2[100][100];
	int i = 0;
	int j;
...
void region_addition() {
	...
	int x, y;
	RRN++;
	printf("추가하실 지역 이름과 좌표를 입력해주세요 >> ");
	scanf("%s %d %d", region, &x, &y);
	fprintf(fp, "%d %s %d %d", RRN - 1, region, x, y);
	file_read();
	fclose(fp);
}
void region_print() {
...
void bookmark_watch() {
	...
	int i = 0;
	int j;
	while (fscanf(fp, "%s", &bookmark[i]) != EOF) {
		i++;
	}
	for (j = 0; j < i; j++) {
		printf("%s\n", bookmark[j]);
